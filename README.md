...
agents/
  code-reviewer.md       # バグ・ロジックエラー・エッジケースのコードレビュー
  doc-reviewer.md        # ドキュメントとコードの乖離を検出
  security-reviewer.md   # 脆弱性・攻撃経路のセキュリティレビュー
hooks/
...
コードの変更内容を読み取り、README.md・開発計画.md などのドキュメントを実態に合わせて自動更新する。実装完了後・コミット前に自動発動。「ドキュメント更新して」でも呼び出し可能。
...
サブエージェントは独立したコンテキストで動作するため、実装セッションのバイアスを持たない客観的なレビューができる。`"use a subagent to ..."` で呼び出す。

### `code-reviewer`

差分を読んでバグ・ロジックエラー・エッジケースを検出する。実装したコードに引きずられないレビューが目的。スタイル指摘・仮定の最適化は行わない。

**呼び出し例**: `"use a subagent to review this diff for bugs"`

### `doc-reviewer`

README や開発計画.md の記述と実コードの乖離を検出する。`/sync-docs` 実行後の確認に使う。

**呼び出し例**: `"use doc-reviewer to check the README against the code"`

### `security-reviewer`

SQLインジェクション・XSS・認証バイパス・シークレット漏洩など、悪用可能な脆弱性を検出する。理論上のリスクより実際の攻撃経路を優先。

**呼び出し例**: `"use security-reviewer on the auth changes before merging"`
